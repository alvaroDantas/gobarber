...
class AppointmentController {
  async index(req, res) { //eslint-disable-line
  ...
  async store(req, res) { // eslint-disable-line
    ...
    /**
     * Check is provider_id is provider
     */
    ...
    if (provider_id === req.userId) {
      return res
        .status(401)
        .json({ error: 'You can not create appointments for itself' });
    }
    ...
    /**
     * Check date availbitiy
     */
    const checkAvailbitiy = await Appointment.findOne({
      where: {
        provider_id,
        canceled_at: null,
        date: hourStart,
  ...
  async delete(req, res) {//eslint-disable-line
